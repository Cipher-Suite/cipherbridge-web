...
export default function LoginPage() {
  ...
  const switchMode = (m) => {
    setMode(m); setError(''); setRegResult(null); setApiKey('');
  };
      ...
      <div style={{ width: '100%', maxWidth: 400, position: 'relative', animation: 'fadeUp 0.5s ease both' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 36 }}>
          <div style={{
            ...
            fontSize: 26, fontWeight: 900, color: T.bg, fontFamily: T.font,
            boxShadow: T.shadowAccent,
          }}>T</div>
          <h1 style={{
            fontFamily: T.font, fontSize: 26, fontWeight: 800,
            color: T.text, letterSpacing: '-0.03em', marginBottom: 6,
          }}>
            Tonpo
          </h1>
          <p style={{ fontFamily: T.font, fontSize: 13, color: T.textDim }}>
            MT5 trading infrastructure platform
          </p>
        </div>
        ...
        <div style={{
          ...
          {/* Mode toggle */}
          <div style={{
            display: 'flex', marginBottom: 24, background: T.bgInput,
            borderRadius: T.radiusSm, padding: 3, gap: 3,
          }}>
            {[['login', 'Sign In'], ['register', 'New Account']].map(([m, label]) => (
              <button key={m} onClick={() => switchMode(m)} style={{
                flex: 1, padding: '9px 0', border: 'none', cursor: 'pointer',
                borderRadius: 6, fontFamily: T.font, fontSize: 12, fontWeight: 700,
          ...
          {mode === 'login' && (
            <form onSubmit={handleLogin} style={{ animation: 'fadeIn 0.2s ease' }}>
              <label style={{
                fontFamily: T.font, fontSize: 11, color: T.textDim, fontWeight: 700,
              ...
              <input
                ...
                style={{
                  ...
                  borderRadius: T.radiusSm, color: T.text, fontFamily: T.mono,
                  fontSize: 14, outline: 'none', transition: T.transition, marginBottom: 8,
                }}
              />
              <p style={{ fontFamily: T.font, fontSize: 12, color: T.textDim, marginBottom: 20, lineHeight: 1.5 }}>
                Your key from <code style={{ color: T.accent, background: T.accentBg, padding: '1px 5px', borderRadius: 4 }}>POST /api/users</code> or the Tonpo dashboard.
              </p>
              ...
              <button type="submit" disabled={loading} style={{
                ...
                color: T.bg, fontFamily: T.font, fontSize: 14, fontWeight: 800,
                cursor: loading ? 'wait' : 'pointer', transition: T.transition,
                display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
              }}>
                {loading && (
                  <span style={{
                    width: 14, height: 14, border: `2px solid ${T.bg}44`,
              ...
              {regResult ? (
                  ...
                  {[['User ID', regResult.userId], ['API Key', regResult.apiKey]].map(([label, val]) => (
                    <div key={label} style={{ marginBottom: 14 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 5 }}>
                        ...
                        {label === 'API Key' && (
                          <button onClick={() => copyKey(val)} style={{
                            background: 'none', border: 'none',
                            color: keyCopied ? T.accent : T.textDim,
                      ...
                      <div style={{
                        padding: '10px 12px', background: T.bgInput,
                        border: `1px solid ${T.border}`, borderRadius: T.radiusSm,
                        fontFamily: T.mono, fontSize: 11, color: T.accent, wordBreak: 'break-all',
                      }}>{val}</div>
                    </div>
                  ))}
                ...
                <>
                  <p style={{
                    ...
                    Creates a new Tonpo account with a fresh API key. No email or password needed — your key <strong style={{ color: T.text }}>is</strong> your identity.
                  </p>
                  ...
                  <button onClick={handleRegister} disabled={loading} style={{
                    ...
                    opacity: loading ? 0.6 : 1, transition: T.transition,
                  }}>
        ...
        <p style={{
          ...
          Secured by Tonpo · Your keys never leave the gateway.
        </p>
      </div>
    </div>
  );
